    ...
    internal class OutPO : Ede.Uof.Utility.Data.BasePersistentObject
    {
        ...
        internal DataTable GetListOut(string D_STEP_DESC, string Type, string LNO, string OutID, string LeaveTime, string ReturnTime, string UserDate1, string UserDate2, string OutCheck, string UserID)
        {
            ...
            string SQL = @" SELECT DISTINCT 
                        CASE 
                            WHEN WF_Out.CheckDate IS NOT NULL THEN 'Da ra ngoai 已外出' 
                            ELSE CASE 
                                WHEN WF_Out.Cancel = 1 THEN 'Da huy 已註銷' 
                                ELSE 'Chua ra ngoai 未外出' 
                            END 
                        END AS Status, 
                        WF_Out.CancelReason, 
                        WF_Out.LNO, 
                        WF_Out.Department, 
                        WF_OutS.People, 
                        CONVERT(VARCHAR, WF_Out.LeaveTime, 120) AS LeaveTime, 
                        CONVERT(VARCHAR, WF_Out.ReturnTime, 120) AS ReturnTime, 
                        WF_Out.Type, 
                        WF_Out.Category, 
                        WF_Out.Item, 
                        WF_Out.Reason, 
                        WF_Out.UserID, 
                        CONVERT(VARCHAR, WF_Out.UserDate, 120) AS UserDate, 
                        BPM.TASK_ID 
                    FROM WF_Out 
                    LEFT JOIN (SELECT LNO AS ListNo, COUNT(ID) AS People FROM WF_OutS GROUP BY LNO) AS WF_OutS 
                        ON WF_OutS.ListNo = WF_Out.LNO 
                    LEFT JOIN (SELECT TASK_ID, DOC_NBR FROM OPENQUERY([BPM_TRINHKY], 'SELECT TASK_ID, DOC_NBR FROM [UOF].[dbo].[TB_WKF_TASK] TB_WKF_TASK') AS SYS_TODOHIS) AS BPM 
                        ON BPM.DOC_NBR COLLATE Chinese_Taiwan_Stroke_CS_AS = WF_Out.LNO COLLATE Chinese_Taiwan_Stroke_CS_AS 
                    LEFT JOIN BUSERS 
                        ON BUSERS.UserID = WF_Out.UserID 
                    WHERE 1=1 " + where + @" 
        ...
        internal string GetLNO()
        {
            string LNO = "";
            string conn = Training.Properties.Settings.Default.LYV_ERP.ToString();
            this.m_db = new Ede.Uof.Utility.Data.DatabaseHelper(conn);
            string cmdTxt = @"SELECT TOP 1 dbo.WF_Out.* FROM dbo.WF_Out ORDER BY LNO DESC";
